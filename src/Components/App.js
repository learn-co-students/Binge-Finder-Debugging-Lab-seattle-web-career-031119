...
class App extends Component {
  ...
  displayShows = () => {
    if (this.state.filterByRating || this.state.searchTerm){
      return this.state.shows.filter((s)=> {
        return s.rating.average >= this.state.filterByRating && s.name.toLowerCase().includes(this.state.searchTerm.toLowerCase())
      })
    } else {
